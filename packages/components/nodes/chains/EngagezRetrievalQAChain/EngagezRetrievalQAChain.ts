...
class EngagezRetrievalQAChain_Chains implements INode {
    label: string
    name: string
    version: number
    type: string
        ...
        if (chainOption === 'map_reduce') {
            obj.qaChainOptions = {
                ...
                combinePrompt: PromptTemplate.fromTemplate(
                    systemMessagePrompt ? `VenueID: ${venueID} User ID: ${userID}\n\n${systemMessagePrompt}\n${map_reduce_template}` : default_map_reduce_template
                )
            } as QAChainParams
        } else if (chainOption === 'refine') {
            const qprompt = new PromptTemplate({
                inputVariables: ['context', 'question'],
                template: refine_question_template(`VenueID: ${venueID} User ID: ${userID}\n\n${systemMessagePrompt}`)
            })
            ...
            obj.qaChainOptions = {
                type: 'stuff',
                prompt: PromptTemplate.fromTemplate(systemMessagePrompt ? `VenueID: ${venueID} User ID: ${userID}\n\n${systemMessagePrompt}\n${qa_template}` : default_qa_template)
            } as QAChainParams
        }
...
module.exports = { nodeClass: EngagezRetrievalQAChain_Chains }
